...
func (inst *Store) GenerateServiceFileAndEdgeUpload(hostUUID, hostName string, app *systemctl.ServiceFile) (*installer.UploadResponse, error) {
	tmpDir, absoluteServiceFileName, err := systemctl.GenerateServiceFile(app, global.App)
	if err != nil {
		return nil, err
	}
	serviceName := global.App.GetServiceNameFromAppName(app.Name)
