...
func (inst *Client) BacnetWhoIs(hostIDName string, body *WhoIsOpts, networkUUID string, addDevices bool) ([]model.Device, error) {
	url := fmt.Sprintf("proxy/ff/api/api/plugins/api/%s/whois/%s?add_devices=%t", bacnetMaster, networkUUID, addDevices)
...
func (inst *Client) BacnetDevicePoints(hostIDName, deviceUUID string, addPoints, makeWriteable bool) ([]model.Point, error) {
	url := fmt.Sprintf("proxy/ff/api/api/plugins/api/%s/device/points/%s?add_points=%t&writeable_points=%t", bacnetMaster, deviceUUID, addPoints, makeWriteable)
	resp, err := nresty.FormatRestyResponse(inst.Rest.R().
		SetHeader("host_uuid", hostIDName).
		SetHeader("host_name", hostIDName).
